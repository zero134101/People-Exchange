...
export async function GET(req: Request) {
  ...
  for (const stock of stocks) {
    ...
    // Update activity metrics from logs
    ...
    if (Math.abs(changeRate) > 20) {
      newsEntries.push({
        ...
        content: `${user.username}의 주가가 ${Math.abs(changeRate).toFixed(1)}% ${changeRate > 0 ? "상승" : "하락"}하여 ${newPrice.toLocaleString()} KRW를 기록했습니다.`,
  ...
  return NextResponse.json({
    ...
    newsGenerated: newsEntries.length,
  })
}
